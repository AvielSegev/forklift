...
var (
	...
	waitForBuildTimeout = time.Minute * 2
)
...
func (h *VddkHandler) ImageUrl(ctx *gin.Context) {
	...
	// return busy in case build process is still running
	if !buildLock.TryLock() {
	...
	JSONSuccess(ctx, fmt.Sprintf("Image: %s exists", registryImageTag), gin.H{"imageUrl": url})
}
...
func BuildAndPushImage() error {
	...
	go waitForBuildAndUnlock(buildClient, namespace, buildObj.Name, waitForBuildTimeout)
...
// waitForBuildAndUnlock will poll your Build every second, stop on a terminal phase
// or after 1 minute, then invoke resetBusy().
func waitForBuildAndUnlock(buildClient buildclientset.Interface, namespace, buildName string, timeout time.Duration) {
